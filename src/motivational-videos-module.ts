...
export default async function Main() {
    ...
    app.post('/videos/setorder', (req, res) => {
        fs.writeFileSync(path.join(__dirname, '../storage/config/clipsOrder.json'), JSON.stringify(req.body, null, 4)); res.sendStatus(200)
    })

    app.get('/open/fs', (req, res) => {
        spawnSync('explorer', [path.join(__dirname, '../storage/videos')]); res.sendStatus(200)
    })
    ...
    app.post('/script/setScript', (req, res) => {
        fs.writeFileSync(path.join(__dirname, '../storage/config/videoScript.txt'), req.body.text); res.sendStatus(200)
    })
    ...
    app.post('/subtitles/setstyle', (req, res) => {
        fs.writeFileSync(path.join(__dirname, '../storage/config/defaultSubtitles.ass'), req.body.text); res.send(200)
    })
