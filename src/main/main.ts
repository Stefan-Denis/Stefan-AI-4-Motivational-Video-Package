...
const unitToTest: 'tts' | 'trim' | 'concat' | 'subtitles' | 'addSubtitles' | 'addAudio' | 'finishing touches' = 'addAudio'
...
class Main {
    static async main() {
        ...
        */
         ...
         */
        // * Global Variables
        const script: Array<string> = await TTS.parseScript()
        const videos: Array<string> = fs.readJsonSync(path.join(__dirname, '../../storage/config/clipsOrder.json'))
        const clipsOrder: Array<string> = JSON.parse(fs.readFileSync(path.join(__dirname, '../../storage/config/clipsOrder.json'), 'utf-8'))
        const audioLengths: Array<number> = []
        ...
        if ((test && unitToTest == 'tts') || !test) {
            ...
            let areScriptLengthsInOrder: boolean = true

            await Promise.all(script.map(async (line: string, index: number) => {
        ...
        if ((test && unitToTest == 'concat') || !test) {
            const videosToConcat: Array<string> = []; videos.forEach((video: string, index: number) => { videosToConcat[index] = path.join(__dirname, `../../storage/temp/video/${video}`) })
     ...
     * @param videoDuration 
     */
    static async displayTTSTimingError(audioDuration: number, videoDuration: number) {
